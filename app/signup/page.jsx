...
export default function Signup() {
  ...
  const validateForm = () => {
    ...
    if (!formData.whatsapp) {
      ...
      newErrors.whatsapp = "Please enter a valid Pakistani mobile number (e.g., +923001234567 or 03001234567)"
    }

    if (!formData.carNumber.trim()) {
      newErrors.carNumber = "Car number is required"
    } else if (!validateCarNumber(formData.carNumber)) {
      newErrors.carNumber = "Please enter a valid car number in format: ABC-123 (3 letters, dash, 3 numbers)"
    }
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-2">Join ParkEase and secure your parking spot</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                required
              />
              {errors.name && <span className="text-red-600 dark:text-red-400 text-sm mt-1">{errors.name}</span>}
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
              />
              {errors.email && <span className="text-red-600 dark:text-red-400 text-sm mt-1">{errors.email}</span>}
            </div>
            ...
            <div>
              ...
              />
              {errors.password && (
                <span className="text-red-600 dark:text-red-400 text-sm mt-1">{errors.password}</span>
              )}
            </div>

            <div>
              <label htmlFor="whatsapp" className="block text-sm font-medium text-black dark:text-white mb-2">
                WhatsApp Number
              </label>
              <input
                type="tel"
                ...
                onChange={handleInputChange}
                className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent text-black dark:text-white bg-white dark:bg-gray-700 ${
                  errors.whatsapp ? "border-red-300" : "border-gray-300 dark:border-gray-600"
                }`}
                placeholder="+923001234567 or 03001234567"
                required
              />
              {errors.whatsapp && (
                <span className="text-red-600 dark:text-red-400 text-sm mt-1">{errors.whatsapp}</span>
              )}
              <small className="text-gray-600 dark:text-gray-400">Pakistani mobile number for confirmation</small>
            </div>

            <div>
              <label htmlFor="carNumber" className="block text-sm font-medium text-black dark:text-white mb-2">
                Car Number
              </label>
              <input
                type="text"
                ...
                onChange={handleInputChange}
                className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white focus:border-transparent text-black dark:text-white bg-white dark:bg-gray-700 ${
                  errors.carNumber ? "border-red-300" : "border-gray-300 dark:border-gray-600"
                }`}
              ...
              />
              {errors.carNumber && (
                <span className="text-red-600 dark:text-red-400 text-sm mt-1">{errors.carNumber}</span>
              )}
