...
import { Car, Calendar, User, LogOut, Plus, MapPin, HelpCircle, Phone, Mail, MessageCircle } from "lucide-react"
...
import SweetAlertProvider, { showSuccess, showError, showWarning } from "@/components/SweetAlert"
...
export default function UserDashboard() {
  ...
  const getActiveBookingsCount = () => {
    return bookings.filter((booking) => booking.status === "Confirmed" && new Date(booking.endDateTime) > new Date())
      .length
  }
  ...
  const getSlotAvailabilityTime = (slotId) => {
    const slotBookings = bookings.filter(
      (booking) =>
        booking.slot === slotId && booking.status === "Confirmed" && new Date(booking.endDateTime) > new Date(),
    )

    if (slotBookings.length === 0) return null

    const latestEndTime = Math.max(...slotBookings.map((booking) => new Date(booking.endDateTime).getTime()))
  ...
  const handleBookingSubmit = (e) => {
    ...
    if (bookingForm.whatsapp && !validatePakistaniNumber(bookingForm.whatsapp)) {
      showError("Please enter a valid Pakistani mobile number (e.g., +923001234567 or 03001234567)")
      return
    }

    // Validate car number
    if (bookingForm.carNumber && !validateCarNumber(bookingForm.carNumber)) {
      showError("Please enter a valid car number in format: ABC-123 (3 letters, dash, 3 numbers)")
      return
    }
  ...
  const handleSlotSelection = (e) => {
    ...
    if (selectedSlot && !selectedSlot.available) {
      ...
      showWarning(
        `Slot ${selectedSlotId} is currently occupied and will be free in ${minutesUntilFree || "Unknown"} minutes. Please select another slot or wait.`,
      )
      return
    }
  ...
  const renderContent = () => {
    switch (activeSection) {
          ...
          <div className="space-y-4 md:space-y-6">
            ...
            <div className="bg-gradient-to-r from-green-500 to-green-600 p-4 md:p-6 rounded-lg shadow-md text-white">
              <div className="flex flex-col md:flex-row items-center justify-between">
                <div className="text-center md:text-left mb-3 md:mb-0">
                  <h3 className="text-lg md:text-xl font-semibold mb-1">💰 Special EasyPaisa Offer!</h3>
                  <p className="text-sm md:text-base">Get Rs. 10 discount per hour when you pay with EasyPaisa!</p>
                  <p className="text-xs md:text-sm opacity-90">
                    Example: 2 hours = Rs. 200 → Pay only Rs. 180 with EasyPaisa
                  </p>
                </div>
                <div className="bg-white/20 px-3 py-2 rounded-lg">
                  <span className="text-sm font-semibold">📱 EasyPaisa: 0334-3282332</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4 md:gap-6">
              <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                <h2 className="text-lg md:text-xl font-semibold text-black dark:text-white mb-4">Parking Overview</h2>
                <div className="flex justify-center space-x-6 md:space-x-8 mb-4 md:mb-6">
                  <div className="text-center">
                    ...
                    <div className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Available Slots</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl md:text-3xl font-bold text-red-600 dark:text-red-400">
                      {availableSlots.filter((slot) => !slot.available).length}
                    </div>
                    <div className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Occupied Slots</div>
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    ...
                    >
                      <Car
                        className={`w-3 h-3 md:w-5 md:h-5 mb-1 ${
                          slot.available ? "text-green-800 dark:text-green-300" : "text-red-800 dark:text-red-300"
                        }`}
                      />
                      <span
                ...
                <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">
                  Showing first 24 slots. Total: {availableSlots.length} slots available.
                </p>
              </div>
            </div>
        ...
        return (
          <div className="space-y-4 md:space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <h1 className="text-2xl md:text-3xl font-bold text-black dark:text-white">My Bookings</h1>
              <button
                onClick={() => setShowBookingForm(true)}
            ...
            {bookings.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 p-8 md:p-12 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 text-center">
                ...
                <p className="text-lg md:text-xl text-gray-600 dark:text-gray-300">No bookings found</p>
                  ...
                  >
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 flex-1">
                        <div>
                          <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Slot</p>
                          <p className="font-semibold text-black dark:text-white text-sm md:text-base">
                            {booking.slot}
                          </p>
                        </div>
                        <div className="col-span-1 md:col-span-2 lg:col-span-2">
                          <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Time</p>
                        ...
                        <div>
                          <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Car</p>
                          <p className="font-semibold text-black dark:text-white text-sm md:text-base">
                            {booking.carNumber}
                          </p>
                        </div>
      ...
      case "amenities":
        return (
          <div className="space-y-4 md:space-y-6">
            <h1 className="text-2xl md:text-3xl font-bold text-black dark:text-white">Nearby Amenities</h1>
            <NearbyAmenities />
          </div>
        )
      ...
      case "faq":
        return (
          <div className="space-y-4 md:space-y-6">
            <h1 className="text-2xl md:text-3xl font-bold text-black dark:text-white">Frequently Asked Questions</h1>
            <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
      ...
      case "profile":
        return (
          <div className="space-y-4 md:space-y-6">
            <h1 className="text-2xl md:text-3xl font-bold text-black dark:text-white">Profile</h1>
            <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 max-w-2xl">
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-base md:text-lg text-black dark:text-white">{userData?.name}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600 dark:text-gray-400">Email</label>
                  <p className="text-base md:text-lg text-black dark:text-white">{userData?.email}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600 dark:text-gray-400">WhatsApp</label>
                  <p className="text-base md:text-lg text-black dark:text-white">{userData?.whatsapp}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600 dark:text-gray-400">Car Number</label>
                  <p className="text-base md:text-lg text-black dark:text-white">{userData?.carNumber}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex min-h-screen">
          <Sidebar className="border-r border-gray-200 dark:border-gray-700 hidden lg:block">
            <SidebarHeader className="p-4 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-lg md:text-xl font-bold text-black dark:text-white">User Panel</h2>
            </SidebarHeader>
          ...
          <SidebarInset className="flex-1">
            <header className="flex h-14 md:h-16 items-center justify-between gap-2 border-b border-gray-200 dark:border-gray-700 px-4 md:px-6 bg-white dark:bg-gray-800">
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-base md:text-lg font-semibold text-black dark:text-white">User Dashboard</h1>
              </div>
              <ThemeToggle />
            </header>
          ...
          <div className="grid grid-cols-5 gap-1 p-2">
            <button
              ...
              className={`flex flex-col items-center p-2 rounded-lg ${
                activeSection === "overview" ? "bg-blue-100 dark:bg-blue-900" : ""
              }`}
            >
              <Car className="w-5 h-5 text-black dark:text-white" />
              <span className="text-xs text-black dark:text-white mt-1">Overview</span>
            </button>
            <button
              onClick={() => setActiveSection("bookings")}
              className={`flex flex-col items-center p-2 rounded-lg ${
                activeSection === "bookings" ? "bg-blue-100 dark:bg-blue-900" : ""
              }`}
            >
              <Calendar className="w-5 h-5 text-black dark:text-white" />
              <span className="text-xs text-black dark:text-white mt-1">Bookings</span>
            </button>
            <button
              onClick={() => setActiveSection("amenities")}
              className={`flex flex-col items-center p-2 rounded-lg ${
                activeSection === "amenities" ? "bg-blue-100 dark:bg-blue-900" : ""
              }`}
            >
              <MapPin className="w-5 h-5 text-black dark:text-white" />
              <span className="text-xs text-black dark:text-white mt-1">Amenities</span>
            </button>
            <button
              onClick={() => setActiveSection("faq")}
              className={`flex flex-col items-center p-2 rounded-lg ${
            ...
            >
              ...
              <span className="text-xs text-black dark:text-white mt-1">FAQ</span>
            </button>
            <button
              onClick={() => setActiveSection("profile")}
              className={`flex flex-col items-center p-2 rounded-lg ${
                activeSection === "profile" ? "bg-blue-100 dark:bg-blue-900" : ""
              }`}
            >
              <User className="w-5 h-5 text-black dark:text-white" />
              <span className="text-xs text-black dark:text-white mt-1">Profile</span>
            </button>
          </div>
        </div>
        ...
        {showBookingForm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
              <h2 className="text-lg md:text-xl font-bold text-black dark:text-white mb-4">Book Parking Slot</h2>
                    ...
                    {availableSlots.map((slot) => {
                      ...
                      return (
                        <option key={slot.id} value={slot.id} disabled={!slot.available}>
                          {slot.id} {!slot.available ? `(Free in ${minutesUntilFree || "Unknown"} min)` : "(Available)"}
                        </option>
                      )
                    })}
                  </select>
                  {bookingForm.slot && !availableSlots.find((s) => s.id === bookingForm.slot)?.available && (
                    <small className="text-orange-600 dark:text-orange-400">
                      This slot will be free in {getSlotAvailabilityTime(bookingForm.slot) || "Unknown"} minutes
                    </small>
                  )}
                </div>
                  ...
                  <div className="flex space-x-4">
                    <label className="flex items-center">
                      <input
                        ...
                        className="mr-2"
                      />
                      <span className="text-black dark:text-white text-sm">💵 Cash</span>
                    </label>
                    <label className="flex items-center">
                      <input
                        type="radio"
                        ...
                        className="mr-2"
                      />
                      <span className="text-black dark:text-white text-sm">📱 EasyPaisa</span>
                    </label>
                  </div>
                </div>
        ...
        {showReceipt && currentBooking && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-lg max-w-lg w-full max-h-[90vh] overflow-y-auto">
              <h2 className="text-lg md:text-xl font-bold text-black dark:text-white mb-4">Booking Details</h2>
