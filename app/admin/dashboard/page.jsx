...
import { Users, Calendar, Settings, BarChart3, Plus, Trash2, Car } from "lucide-react"
...
import SweetAlertProvider, { showSuccess, showError } from "@/components/SweetAlert"
...
export default function AdminDashboard() {
  ...
  const saveEditedBooking = (e) => {
    ...
    if (editingBooking.carNumber && !validateCarNumber(editingBooking.carNumber)) {
      showError("Please enter a valid car number in format: ABC-123 (3 letters, dash, 3 numbers)")
      return
    }
      ...
      prev.map((booking) =>
        booking.id === editingBooking.id
          ? {
              ...editingBooking,
              hours: diffHours,
              totalAmount: diffHours * PRICE_PER_HOUR, // Admin always pays full price
            }
          : booking,
      ),
    )
  ...
  const handleAdminEasyPaisaComplete = () => {
    ...
    switch (activeSection) {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400">Active Bookings</h3>
                <p className="text-3xl font-bold text-black dark:text-white">
                  {bookings.filter((b) => b.status === "Active").length}
                </p>
              </div>
              <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400">Available Slots</h3>
                <p className="text-3xl font-bold text-black dark:text-white">
                  {slots.filter((s) => s.available).length}
                </p>
              </div>
              <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400">Price Per Hour</h3>
                <p className="text-3xl font-bold text-black dark:text-white">Rs. {PRICE_PER_HOUR}</p>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-black dark:text-white">Parking Slots (A1 - A80)</h2>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Showing first 40 slots.</p>
            </div>
          </div>
        )
                  ...
                  {filteredUsers.map((user) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-gray-600 dark:text-gray-400">WhatsApp: {user.whatsapp}</p>
                        <p className="text-gray-600 dark:text-gray-400">Car: {user.carNumber}</p>
                        <p className="text-gray-600 dark:text-gray-400">Joined: {user.joinDate}</p>
                      </div>
                      <button
      ...
      case "bookings":
        ...
        return (
          <div className="space-y-6">
            <div className="flex justify-between items-center gap-4">
              <h1 className="text-3xl font-bold text-black dark:text-white">Bookings Management</h1>
              <button
                  ...
                  {filteredBookings.map((booking) => (
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 flex-1">
                        <div>
                          <p className="text-sm text-gray-600 dark:text-gray-400">ID: {booking.id}</p>
                          <p className="text-sm text-gray-600 dark:text-gray-400">User: {booking.userName}</p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600 dark:text-gray-400">Slot: {booking.slot}</p>
                          <p className="text-sm text-gray-600 dark:text-gray-400">Car: {booking.carNumber}</p>
                        </div>
                        <div className="col-span-1 md:col-span-2 lg:col-span-1">
                          <p className="text-sm text-gray-600 dark:text-gray-400">
                            Start: {formatDateTime(booking.startDateTime)}
                          </p>
                          <p className="text-sm text-gray-600 dark:text-gray-400">
                            End: {formatDateTime(booking.endDateTime)}
                          </p>
                        </div>
                        <div>
                          <span
                          ...
                          <p className="text-sm font-semibold text-black dark:text-white">Rs. {booking.totalAmount}</p>
                        </div>
                      </div>
  ...
  const getSlotAvailabilityTime = (slotId) => {
    const slotBookings = bookings.filter(
      (booking) => booking.slot === slotId && booking.status === "Active" && new Date(booking.endDateTime) > new Date(),
    )
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <SidebarProvider>
        ...
        {showAddUserForm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-md w-full mx-4">
              <h2 className="text-xl font-bold text-black dark:text-white mb-4">Add New User</h2>
              <form onSubmit={handleAddUser} className="space-y-4">
                ...
                <div>
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded text-black dark:text-white bg-white dark:bg-gray-700"
                  />
                  <small className="text-gray-600 dark:text-gray-400">Pakistani mobile number</small>
                </div>
                <div>
                  <input
                    type="text"
        ...
        {showEditBookingForm && editingBooking && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
              <h2 className="text-xl font-bold text-black dark:text-white mb-4">Edit Booking</h2>
              <form onSubmit={saveEditedBooking} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-black dark:text-gray-300 mb-2">Slot</label>
                  <input
                    type="text"
                ...
                <DateTimeRangePicker
                  ...
                  onStartChange={(value) => setEditingBooking({ ...editingBooking, startDateTime: value })}
                  onEndChange={(value) => setEditingBooking({ ...editingBooking, endDateTime: value })}
                />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingBooking({ ...editingBooking, carNumber: e.target.value.toUpperCase() })}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded text-black dark:text-white bg-white dark:bg-gray-700"
                    placeholder="ABC-123"
                    maxLength={7}
                    required
                  />
                  <small className="text-gray-600 dark:text-gray-400">Format: ABC-123</small>
                </div>
                <div>
                  <label className="block text-sm font-medium text-black dark:text-gray-300 mb-2">Status</label>
                  <select
        ...
        {showAddBookingForm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
              <h2 className="text-xl font-bold text-black dark:text-white mb-4">Add Manual Booking</h2>
              <form onSubmit={handleAddBooking} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-black dark:text-gray-300 mb-2">Select User *</label>
                  <select
                    name="userId"
                ...
                <div>
                  ...
                  {newBooking.slot && !slots.find((s) => s.id === newBooking.slot)?.available && (
                    <small className="text-orange-600 dark:text-orange-400">
                      This slot will be free in {getSlotAvailabilityTime(newBooking.slot) || "Unknown"} minutes
                    </small>
                  )}
                </div>
                ...
                <DateTimeRangePicker
                  ...
                  onStartChange={(value) => setNewBooking({ ...newBooking, startDateTime: value })}
                  onEndChange={(value) => setNewBooking({ ...newBooking, endDateTime: value })}
                />
              ...
              <div className="space-y-4 mb-6">
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    <p className="text-gray-700 dark:text-gray-300">
                      <strong>Start Time:</strong> {formatDateTime(selectedBooking.startDateTime)}
                    </p>
                    <p className="text-gray-700 dark:text-gray-300">
                      <strong>End Time:</strong> {formatDateTime(selectedBooking.endDateTime)}
                    </p>
                    <p className="text-gray-700 dark:text-gray-300">
                      <strong>Duration:</strong> {selectedBooking.hours} hour(s)
                    </p>
                    <p className="text-gray-700 dark:text-gray-300">
                    ...
                    <p className="text-gray-700 dark:text-gray-300">
                      ...
                      {selectedBooking.paymentMethod === "easypaisa" ? "EasyPaisa" : "Cash"}
                    </p>
                    <p className="text-gray-700 dark:text-gray-300">
